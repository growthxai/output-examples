...
export const scoreDimension = step( {
  ...
  fn: async ( { transcript, methodology, dimension } ) => {
    ...
    const { output } = await generateText( {
      ...
      output: Output.object( { schema: dimensionScoreSchema } )
    } );

    return output as DimensionScore;
  }
} );
...
export const synthesizeResults = step( {
  ...
  fn: async ( { methodology, dimensionScores } ) => {
    const scoresText = dimensionScores.map( ( d ) =>
      `### ${d.dimension}: ${d.score}/10\n- Evidence: ${d.evidence}${d.gap ? `\n- Gap: ${d.gap}` : ''}${d.recommendation ? `\n- Recommendation: ${d.recommendation}` : ''}`
    ).join( '\n\n' );
